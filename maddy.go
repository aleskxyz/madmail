...
func init() {
	// Wire the compile-time ConfigDirectory into the framework/config package
	// so that subpackages can resolve config paths without an import cycle.
	config.EffectiveConfigDir = func() string { return ConfigDirectory }
}
...
default runtime_dir: %s`,
		...
		config.ConfigFile(),
		config.DefaultStateDirPath(),
		config.DefaultRuntimeDirPath())
}
...
func init() {
	...
	maddycli.AddSubcommand(&cli.Command{
		...
		Flags: []cli.Flag{
			&cli.StringFlag{
				Name:        "libexec",
				Value:       config.DefaultLibexecDirPath(),
...
func InitDirs() error {
	if config.StateDirectory == "" {
		config.StateDirectory = config.DefaultStateDirPath()
		if DefaultStateDirectory != "" {
			config.StateDirectory = DefaultStateDirectory
		}
	}
	if config.RuntimeDirectory == "" {
		config.RuntimeDirectory = config.DefaultRuntimeDirPath()
		if DefaultRuntimeDirectory != "" {
			config.RuntimeDirectory = DefaultRuntimeDirectory
		}
	}
	if config.LibexecDirectory == "" {
		config.LibexecDirectory = config.DefaultLibexecDirPath()
		if DefaultLibexecDirectory != "" {
...
func ReadGlobals(cfg []config.Node) (map[string]interface{}, []config.Node, error) {
	...
	globals.String("state_dir", false, false, config.DefaultStateDirPath(), &config.StateDirectory)
	globals.String("runtime_dir", false, false, config.DefaultRuntimeDirPath(), &config.RuntimeDirectory)
